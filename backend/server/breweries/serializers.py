...
import os
from .models import Brewery, BreweryImage, ChecklistItem, Collection, Note, Transportation, Checklist, Visit, Category, Attachment
from rest_framework import serializers
from main.utils import CustomModelSerializer


class BreweryImageSerializer(CustomModelSerializer):
    class Meta:
        model = BreweryImage
        fields = ['id', 'image', 'brewery', 'is_primary', 'user_id']
...
class AttachmentSerializer(CustomModelSerializer):
    ...
    class Meta:
        model = Attachment
        fields = ['id', 'file', 'brewery', 'extension', 'name', 'user_id']
        read_only_fields = ['id', 'user_id']
...
class CategorySerializer(serializers.ModelSerializer):
    num_breweries = serializers.SerializerMethodField()
    class Meta:
        model = Category
        fields = ['id', 'name', 'display_name', 'icon', 'num_breweries']
        read_only_fields = ['id', 'num_breweries']
    ...
    def get_num_breweries(self, obj):
        return Brewery.objects.filter(category=obj, user_id=obj.user_id).count()
...
class BrewerySerializer(CustomModelSerializer):
    images = BreweryImageSerializer(many=True, read_only=True)
    ...
    def create(self, validated_data):
        ...
        print(category_data)
        brewery = Brewery.objects.create(**validated_data)
        for visit_data in visits_data:
            Visit.objects.create(brewery=brewery, **visit_data)

        if category_data:
            category = self.get_or_create_category(category_data)
            brewery.category = category
            brewery.save()
        ...
        if has_visits:
            ...
            for visit_data in visits_data:
                ...
                if visit_id and visit_id in current_visit_ids:
                    ...
                    new_visit = Visit.objects.create(brewery=instance, **visit_data)
...
class CollectionSerializer(CustomModelSerializer):
    breweries = BrewerySerializer(many=True, read_only=True, source='brewery_set')
    ...
    class Meta:
        model = Collection
        fields = ['id', 'description', 'user_id', 'name', 'is_public', 'breweries', 'created_at', 'start_date', 'end_date', 'transportations', 'notes', 'updated_at', 'checklists', 'is_archived', 'shared_with', 'link']
        read_only_fields = ['id', 'created_at', 'updated_at', 'user_id']
