...
class BreweryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """
        Returns the queryset for the BreweryViewSet. Unauthenticated users can only
        retrieve public breweries, while authenticated users can access their own,
        shared, and public breweries depending on the action.
        """
        user = self.request.user

        if not user.is_authenticated:
            # Unauthenticated users can only access public breweries for retrieval
            if self.action == 'retrieve':
                return Brewery.objects.retrieve_breweries(user, include_public=True).order_by('-updated_at')
            return Brewery.objects.none()

        # Authenticated users: Handle retrieval separately
        include_public = self.action == 'retrieve'
        return Brewery.objects.retrieve_breweries(
            user,
