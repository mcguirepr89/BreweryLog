<script lang="ts">
	...
	brewery = {
		...
		attachments: breweryToEdit?.attachments || []
	};
	...
	function deleteAttachment(event: CustomEvent<string>) {
		brewery.attachments = brewery.attachments.filter(
	...
	async function editAttachment() {
		if (attachmentToEdit) {
			...
			if (res.ok) {
				...
				brewery.attachments = brewery.attachments.map((attachment) => {
				...
				addToast('success', $t('breweries.attachment_update_success'));
			} else {
				addToast('error', $t('breweries.attachment_update_error'));
			}
		}
	}
	...
	async function uploadAttachment(event: Event) {
		...
		try {
			const res = await fetch('/breweries?/attachment', {
				method: 'POST',
				body: formData
			});
			...
			if (res.ok) {
				...
				brewery.attachments = [...brewery.attachments, newData.data];
				addToast('success', $t('breweries.attachment_upload_success'));
				attachmentName = '';
			} else {
				addToast('error', $t('breweries.attachment_upload_error'));
			}
		} catch (err) {
			console.error(err);
			addToast('error', $t('breweries.attachment_upload_error'));
		...
		{:else}
			<div class="modal-action items-center">
				<div class="collapse collapse-plus bg-base-200 mb-4">
					<input type="checkbox" />
					<div class="collapse-title text-xl font-medium">
						{$t('breweries.attachments')} ({brewery.attachments?.length || 0})
					</div>
					<div class="collapse-content">
						...
						<div class="flex gap-2 m-4">
							...
							<input
								type="text"
								class="input input-bordered w-full"
								placeholder={$t('breweries.attachment_name')}
			...
			<div class="collapse collapse-plus bg-base-200 mb-4">
				<input type="checkbox" checked />
				<div class="collapse-title text-xl font-medium">
					{$t('breweries.images')} ({brewery.images?.length || 0})
				</div>
				<div class="collapse-content">
					...
					<div class="mb-4">
						...
						{#if wikiImageError}
							<p class="text-red-500">{$t('breweries.wiki_image_error')}</p>
						{/if}
					</div>
